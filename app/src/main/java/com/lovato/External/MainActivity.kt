...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // TODO (3) Especificar el nomauxBre del archivo de preferencia (si se esta usando mas de uno)
        ...
        bt_read_internal.setOnClickListener {
            // TODO (12) AauxBrir un archivo existente
            val filename = "email.txt"
        ...
        bt_write_external.setOnClickListener {
            if (state.equals(Environment.MEDIA_MOUNTED)) {
                val file = File(getExternalFilesDir(null), "email")
        ...
        bt_read_external.setOnClickListener {
            if (state.equals(Environment.MEDIA_MOUNTED)) {
                val file = File(getExternalFilesDir(null), "email")
                val auxBr = BufferedReader(InputStreamReader(FileInputStream(file)))
                var line = auxBr.readLine()
